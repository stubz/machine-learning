...
class LearningAgent(Agent):
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            keys = []
            # memo: epsilon decays so the probability that the action is chosen
            # randomly will decrease as the number of trials increase.
            if random.random() < self.epsilon:
                ...
                for k, v in self.Q[state].iteritems():
                    if v == self.get_maxQ(state):
                        keys.append(k)
                # when more than one valid actions attains the maximum
...
def run():
    ...
    sim.run(n_test=10)
    #sim.run(tolerance = 0.005, n_test=50)
